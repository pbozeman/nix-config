...
# requiring single user mode to fix.  At a minimum, we should query brew to figure
# out where the module was installed, and maybe add some other safeguards.
  let
      ...
      ${
        ...
        then ''
          ...
          if ! grep 'pam_tid.so' ${file} > /dev/null; then
            /usr/bin/sed -i "" '2i\
          auth       optional       /usr/local/Cellar/pam-reattach/1.3/lib/pam/pam_reattach.so # nix-darwin: ${option}\
